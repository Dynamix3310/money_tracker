            ...
            <div className="max-w-2xl mx-auto p-4 space-y-6">
               {activeTab === 'home' && (
                  <div className="space-y-4 animate-in slide-in-from-bottom-4">
                     <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 h-64"><h3 className="font-bold text-slate-700 text-sm mb-2 flex items-center gap-2"><LineChart size={16} /> 資產趨勢</h3><NetWorthAreaChart data={useMemo(() => historyData.map(h => ({ label: safeDate(h.date), value: h.amount })).slice(-14), [historyData])} /></div>
                     <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 h-64"><h3 className="font-bold text-slate-700 text-sm mb-4 flex items-center gap-2"><TrendingUp size={16} /> 收支分析</h3><CashFlowBarChart data={useMemo(() => getMonthlyCashFlow(transactions, baseCurrency, rates), [transactions, baseCurrency, rates])} /></div>
                  </div>
               )}
