...
export default function App() {
   ...
   const [enableSuccessAnimation, setEnableSuccessAnimation] = useState(localStorage.getItem('enable_success_animation') !== 'false');
   const toggleSuccessAnimation = () => { const newVal = !enableSuccessAnimation; setEnableSuccessAnimation(newVal); localStorage.setItem('enable_success_animation', String(newVal)); };
   ...
   return (
      <div className="flex flex-col h-screen bg-slate-50 text-slate-900 font-sans overflow-hidden relative">
         ...
         {activeModal === 'settings' && <SettingsModal onClose={() => setActiveModal(null)} onExport={exportData} onExportCSV={exportCSV} onImport={handleImport} onGroupJoin={handleGroupJoin} onGroupCreate={handleCreateGroup} onGroupSwitch={handleSwitchGroup} currentGroupId={currentGroupId} groups={userGroups} user={user} categories={categories} onAddCategory={(name: string, type: string, budget: number) => addDoc(collection(db, getCollectionPath(user!.uid, currentGroupId, 'categories')), { name, type, budgetLimit: budget || 0 })} onUpdateCategory={(id: string, data: any) => updateDoc(doc(db, getCollectionPath(user!.uid, currentGroupId, 'categories'), id), data)} onDeleteCategory={(id: string) => confirmDelete(async () => await deleteDoc(doc(db, getCollectionPath(user!.uid, currentGroupId, 'categories'), id)), '確定刪除此分類? (需二次確認)')} currentTheme={themeColor} onSetTheme={setThemeColor} enableSuccessAnimation={enableSuccessAnimation} onToggleSuccessAnimation={toggleSuccessAnimation} />}
         {(activeModal === 'add-trans' || activeModal === 'edit-trans') && <AddTransactionModal userId={user?.uid} groupId={currentGroupId} people={people} categories={categories} onClose={() => { setActiveModal(null); setSelectedItem(null) }} editData={selectedItem} rates={rates} convert={convert} accounts={calculatedAccounts} enableSuccessAnimation={enableSuccessAnimation} />}
