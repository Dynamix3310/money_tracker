...
import { X, Download, Share2, Trash2, Camera, Loader2, ArrowUpRight, ArrowDownRight, Sparkles, Send, CheckCircle, Circle, Link as LinkIcon, Link2, Upload, ArrowRightLeft, Save, RefreshCw, Building2, Wallet, Landmark, Edit, Key, Settings, Repeat, AlertCircle, FileText, Image as ImageIcon, CreditCard } from 'lucide-react';
...
export const AIBatchImportModal = ({ userId, groupId, categories, existingTransactions, existingBankLogs, existingCardLogs, accounts, creditCards, people, onClose }: any) => {
   ...
   // Auto-select first valid option when type changes
   useEffect(() => {
       if (importType === 'bank' && accounts.length > 0) setTargetId(accounts[0].id);
       else if (importType === 'card' && creditCards.length > 0) setTargetId(creditCards[0].id);
       else setTargetId('');
   }, [importType, accounts, creditCards]);

   const checkDuplicates = (items: any[]) => {
       let pool: any[] = [];
       if (importType === 'ledger') pool = existingTransactions;
       else if (importType === 'bank') pool = existingBankLogs.filter((l:any) => l.accountId === targetId);
       else if (importType === 'card') pool = existingCardLogs.filter((l:any) => l.cardId === targetId);

       return items.map((item, idx) => {
           const isDup = pool.some((ex: any) => {
               const exDate = ex.date?.seconds ? new Date(ex.date.seconds * 1000).toISOString().split('T')[0] : '';
               const exAmt = ex.amount || ex.totalAmount || 0;
               const exDesc = ex.description || '';
               
               const dateMatch = exDate === item.date;
               const amtMatch = Math.abs(exAmt - parseFloat(item.amount)) < 1;
               const descMatch = exDesc.includes(item.description.substring(0,5)) || item.description.includes(exDesc.substring(0,5));
               
               return dateMatch && amtMatch && descMatch;
           });
           return { ...item, id: idx, checked: !isDup, isDuplicate: isDup };
       });
   };
   ...
   const handleAIParse = async (imageBase64?: string) => {
      ...
      try {
         ...
         const prompt = `
            ...
            Return JSON Array only. Items:
            - date (YYYY-MM-DD)
            - amount (number, positive)
            - description (string)
            - type ("expense" or "income") ${importType === 'card' ? '(Usually expense)' : ''}
            ${importType === 'ledger' ? '- category (String from list or "Other")' : ''}
         ...
         if (Array.isArray(json)) {
             setParsedItems(checkDuplicates(json));
             setMode('review');
         } else {
             alert('AI 無法辨識內容，請重試');
         }
   ...
   const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (file) {
          ...
          reader.onloadend = () => handleAIParse(reader.result as string);
          reader.readAsDataURL(file);
      }
   };

   const handleSave = async () => {
      const toAdd = parsedItems.filter(i => i.checked);
      if (toAdd.length === 0) return;
      
      setLoading(true);
      const payerId = people[0]?.id || 'unknown';
      
      try {
         if (importType === 'ledger') {
             const col = collection(db, getCollectionPath(userId, groupId, 'transactions'));
             for (const item of toAdd) {
                 const finalAmt = parseFloat(item.amount);
                 const splitAmt = finalAmt / (people.length || 1);
                 const splits: any = {}; people.forEach((p:any) => splits[p.id] = splitAmt);
                 await addDoc(col, {
         ...
         } else if (importType === 'card') {
             const col = collection(db, getCollectionPath(userId, null, 'cardLogs'));
             for (const item of toAdd) {
                 await addDoc(col, {
                     cardId: targetId,
                     amount: parseFloat(item.amount), // Assume positive for log
                     description: item.description,
                     date: Timestamp.fromDate(new Date(item.date)),
                     isReconciled: false
                 });
   ...
   return (
      <div className={styles.overlay}>
         <div className={`${styles.content} w-full max-w-2xl`}>
            <div className="flex justify-between items-center mb-4">
               <h3 className="font-bold text-xl flex items-center gap-2"><Sparkles className="text-indigo-500"/> AI 批量匯入</h3>
               <button onClick={onClose}><X size={20}/></button>
            </div>
            ...
            {mode === 'input' ? (
               <div className="space-y-4">
                  <div className="bg-slate-100 p-1 rounded-xl flex text-xs font-bold">
                      <button onClick={()=>setImportType('ledger')} className={`flex-1 py-2 rounded-lg flex items-center justify-center gap-1 ${importType==='ledger'?'bg-white shadow text-indigo-600':'text-slate-400'}`}><FileText size={14}/> 記帳</button>
                      <button onClick={()=>setImportType('bank')} className={`flex-1 py-2 rounded-lg flex items-center justify-center gap-1 ${importType==='bank'?'bg-white shadow text-indigo-600':'text-slate-400'}`}><Landmark size={14}/> 銀行</button>
                      <button onClick={()=>setImportType('card')} className={`flex-1 py-2 rounded-lg flex items-center justify-center gap-1 ${importType==='card'?'bg-white shadow text-indigo-600':'text-slate-400'}`}><CreditCard size={14}/> 信用卡</button>
                  </div>
                  ...
                  <textarea 
                     ...
                     placeholder={importType==='ledger' ? "輸入消費內容，例如：午餐 100, 晚餐 250..." : "貼上交易明細，例如：2023/10/01 轉帳 5000..."}
                     value={text}
                     onChange={e => setText(e.target.value)}
                  />
                  <div className="grid grid-cols-2 gap-4">
                     <button onClick={() => handleAIParse()} disabled={!text || loading || (importType!=='ledger' && !targetId)} className={styles.btnPrimary}>
                        {loading ? <Loader2 className="animate-spin"/> : <FileText/>} 
                        解析文字
                     </button>
                     <label className={`${styles.btnSecondary} cursor-pointer flex items-center justify-center gap-2 ${loading || (importType!=='ledger' && !targetId) ? 'opacity-50 cursor-not-allowed' : ''}`}>
                         {loading ? <Loader2 className="animate-spin"/> : <Camera/>} 
                         拍照/上傳
                         <input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} disabled={loading || (importType!=='ledger' && !targetId)}/>
                     </label>
                  </div>
               </div>
            ) : (
               <div className="space-y-4">
                  <div className="flex justify-between items-center">
                      <div className="text-sm font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded">
                          匯入至: {importType==='ledger' ? '記帳本' : importType==='bank' ? accounts.find((a:any)=>a.id===targetId)?.name : creditCards.find((c:any)=>c.id===targetId)?.name}
                      </div>
                  </div>
                  <div className="max-h-[50vh] overflow-y-auto space-y-2">
                     {parsedItems.map((item, idx) => (
                        <div key={idx} className={`flex items-center gap-3 p-3 rounded-xl border ${item.isDuplicate ? 'bg-red-50 border-red-200' : 'bg-slate-50 border-slate-200'}`}>
                           <input 
                              type="checkbox" 
                           ...
                           <div className="flex-1 grid grid-cols-12 gap-2 items-center">
                              <input className="col-span-3 bg-transparent text-sm border-b border-dashed border-slate-300 focus:border-indigo-500 outline-none" value={item.date} onChange={e=>{const n=[...parsedItems];n[idx].date=e.target.value;setParsedItems(n)}} type="date"/>
                              <input className="col-span-4 bg-transparent text-sm font-bold border-b border-dashed border-slate-300 focus:border-indigo-500 outline-none" value={item.description} onChange={e=>{const n=[...parsedItems];n[idx].description=e.target.value;setParsedItems(n)}}/>
                              <div className="col-span-3 flex items-center gap-1">
                                  <span className="text-xs text-slate-400">$</span>
                                  <input className="w-full bg-transparent text-sm font-mono font-bold border-b border-dashed border-slate-300 focus:border-indigo-500 outline-none" value={item.amount} onChange={e=>{const n=[...parsedItems];n[idx].amount=e.target.value;setParsedItems(n)}}/>
                              </div>
                              {importType === 'ledger' && <div className="col-span-2 text-xs text-slate-500 bg-white px-1 rounded border text-center truncate">{item.category}</div>}
                              {importType !== 'ledger' && <div className={`col-span-2 text-[10px] text-center px-1 rounded font-bold ${item.type==='income'?'bg-emerald-100 text-emerald-600':'bg-red-100 text-red-500'}`}>{item.type||'expense'}</div>}
                           </div>
                           {item.isDuplicate && (
                               <div className="group relative">
                                   <AlertCircle size={18} className="text-red-500"/>
                                   <div className="absolute right-0 bottom-full mb-2 bg-black text-white text-xs p-2 rounded w-32 hidden group-hover:block">疑似重複資料</div>
                               </div>
                           )}
                        </div>
                     ))}
                  ...
                  <div className="flex gap-3 pt-2">
                     <button onClick={() => setMode('input')} className={styles.btnSecondary}>重來</button>
                     <button onClick={handleSave} className={`${styles.btnPrimary} flex-1`}>
                        儲存選取項目 ({parsedItems.filter(i => i.checked).length})
                     </button>
                  </div>
               </div>
            )}
...
export const AIAssistantModal = ({ onClose, contextData }: any) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-[70] flex flex-col justify-end sm:justify-center sm:items-center">
       <div className="bg-white w-full sm:max-w-md sm:rounded-2xl h-[85vh] sm:h-[600px] flex flex-col animate-in slide-in-from-bottom-10 shadow-2xl overflow-hidden">
          <div className="p-4 border-b flex justify-between items-center bg-indigo-600 text-white sm:rounded-t-2xl">
             <div className="flex items-center gap-2 font-bold"><Sparkles size={18}/> 財務管家 AI</div>
             <button onClick={onClose}><X size={20}/></button>
          </div>
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50">
             {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                   <div className={`max-w-[85%] p-3 rounded-2xl text-sm ${msg.role === 'user' ? 'bg-indigo-600 text-white' : 'bg-white border'}`}>{msg.text}</div>
                </div>
             ))}
             {thinking && <div className="text-xs text-slate-400 ml-4">思考中...</div>}
          </div>
          <div className="p-3 border-t bg-white flex gap-2">
             <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSend()} placeholder="輸入問題..." className="flex-1 bg-slate-100 rounded-full px-4 py-2 text-sm" />
             <button onClick={handleSend} disabled={thinking} className="p-2 bg-indigo-600 text-white rounded-full"><Send size={18} /></button>
          </div>
       </div>
    </div>
  )
...
export const SettingsModal = ({ onClose, onExport, onImport, onGroupJoin, currentGroupId, categories, onAddCategory, onDeleteCategory }: any) => {
    ...
    <div className={styles.overlay}>
      <div className={styles.content}>
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl mb-6">
           <button onClick={() => setActiveTab('general')} className={`flex-1 py-2 text-xs font-bold rounded-lg ${activeTab === 'general' ? 'bg-white shadow' : 'text-slate-400'}`}>一般</button>
           <button onClick={() => setActiveTab('category')} className={`flex-1 py-2 text-xs font-bold rounded-lg ${activeTab === 'category' ? 'bg-white shadow' : 'text-slate-400'}`}>分類管理</button>
        </div>

        {activeTab === 'general' ? (
          <div className="space-y-6">
             <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100">
                <div className="font-bold text-indigo-800 mb-2 flex items-center gap-2"><Share2 size={16}/> 群組同步記帳</div>
                <p className="text-xs text-indigo-600 mb-3 leading-relaxed">
                  將下方的 ID 分享給您的家人或朋友。當他們在自己的裝置上輸入此 ID 並點擊「加入」後，你們的「記帳資料」將會即時同步。
                </p>
                {currentGroupId ? <div className="font-mono bg-white p-2 rounded border border-indigo-200 mb-2 text-sm flex justify-between font-bold select-all">{currentGroupId} <button onClick={()=>onGroupJoin(null)} className="text-red-500 font-bold text-xs">退出</button></div> : 
                <div className="flex gap-2"><input id="gid" placeholder="輸入對方提供的 ID" className="w-full p-2 rounded border text-sm"/><button onClick={()=>onGroupJoin((document.getElementById('gid') as HTMLInputElement).value)} className="bg-indigo-600 text-white px-3 rounded text-xs font-bold">加入</button></div>}
             </div>

             <div>
                <div className="font-bold text-slate-800 mb-2 flex items-center gap-2 text-sm"><Key size={16}/> 股票 API 設定</div>
                <label className={styles.label}>Finnhub API Key (選填)</label>
                <div className="flex gap-2">
                    <input placeholder="預設使用公共 Key" className={`${styles.input} py-2 text-sm`} value={apiKey} onChange={e => setApiKey(e.target.value)} />
                    <button onClick={saveKey} className="bg-slate-800 text-white px-3 rounded-xl font-bold text-xs">儲存</button>
                </div>
                <p className="text-[10px] text-slate-400 mt-1">若股價更新失敗，請至 Finnhub.io 免費申請 Key 並填入。</p>
             </div>

             <div className="grid grid-cols-2 gap-3 pt-4 border-t">
               <button onClick={onExport} className={`${styles.btnSecondary} flex items-center justify-center gap-2`}><Download size={16}/> 匯出 JSON</button>
               <button onClick={() => fileInputRef.current?.click()} className={`${styles.btnSecondary} flex items-center justify-center gap-2`}><Upload size={16}/> 匯入 JSON</button>
...
export const AddTransactionModal = ({ userId, groupId, people, categories, onClose, editData }: any) => {
   ...
   const [payerId, setPayerId] = useState(editData ? Object.keys(editData.payers)[0] : (people[0]?.id || ''));
   ...
   const handleSave = async () => {
      ...
      const splitAmt = finalAmt / (people.length||1);
      const splits: any = {}; people.forEach((p:any) => splits[p.id] = splitAmt);
      const data = { totalAmount: finalAmt, description, category, type, payers: {[payerId]: finalAmt}, splitDetails: splits, date: Timestamp.fromDate(new Date(date)), currency: 'TWD' };
...
export const CardDetailModal = ({ userId, card, cardLogs, transactions, allCardLogs, onClose }: any) => {
   ...
   const handleSaveLog = async () => {
      ...
      if (editingId) {
         ...
         await addDoc(col, { ...payload, isReconciled: false });
      }
   ...
   const handleEditClick = (log: any) => {
      setEditingId(log.id);
      setAmt(log.amount.toString());
      setDesc(log.description);
      setDate(new Date(log.date.seconds * 1000).toISOString().split('T')[0]);
      setShowAddLog(true);
   };

   const handleDeleteClick = async (id: string) => {
      if (window.confirm('確定要刪除此筆刷卡紀錄嗎？(需二次確認)')) {
         await deleteDoc(doc(db, getCollectionPath(userId, null, 'cardLogs'), id));
         if (editingId === id) { setShowAddLog(false); setEditingId(null); }
      }
   };

   const linkTrans = async (transId: string) => {
      if(!linkLog) return;
      await updateDoc(doc(db, getCollectionPath(userId, null, 'cardLogs'), linkLog.id), { isReconciled: true, linkedTransactionId: transId });
      setLinkLog(null);
   }
   ...
   return (
      <div className={styles.overlay}>
         {linkLog ? (
            <div className={styles.content}>
               <div className="flex justify-between items-center mb-4"><h3 className="font-bold">連結記帳資料</h3><button onClick={()=>setLinkLog(null)}><X/></button></div>
                     ...
                     return (
                        <div key={log.id} className={`p-3 rounded-xl border ${log.isReconciled ? 'bg-emerald-50/50 border-emerald-100' : 'bg-white border-slate-200'}`}>
                           ...
                           <div className="flex justify-between items-center pl-7">
                              <div className="text-[10px] text-slate-400">{new Date(log.date.seconds*1000).toLocaleDateString()}</div>
                              <div className="flex items-center gap-2">
                                {!log.isReconciled && <button onClick={()=>setLinkLog(log)} className="text-[10px] flex gap-1 bg-indigo-50 text-indigo-600 px-2 py-1 rounded font-bold"><LinkIcon size={10}/> 連結</button>}
                                {log.isReconciled && linkedT && <span className="text-[10px] text-emerald-600 flex gap-1 bg-emerald-50 px-2 py-1 rounded"><Link2 size={10}/> {linkedT.description}</span>}
...
export const AddPlatformModal = ({ userId, onClose, editData }: any) => {
   ...
   const handleSave = async () => {
      ...
      if (editData) {
         await updateDoc(doc(col, editData.id), { name, type, currency, balance: parseFloat(initialCash) }); 
      } else {
         await addDoc(col, {
            name, type, balance: parseFloat(initialCash)||0, currency
         });
      }
      onClose();
   };
   ...
   return (
      <div className={styles.overlay}><div className={styles.content}>
         <h3 className="font-bold text-xl mb-4">{editData ? '編輯投資平台' : '新增投資平台'}</h3>
         <div className="space-y-4">
             <div className="flex bg-slate-100 p-1 rounded-xl"><button onClick={()=>setType('stock')} className={`flex-1 py-2 rounded-lg text-sm font-bold ${type==='stock'?'bg-white shadow text-blue-600':'text-slate-400'}`}>證券戶</button><button onClick={()=>setType('crypto')} className={`flex-1 py-2 rounded-lg text-sm font-bold ${type==='crypto'?'bg-white shadow text-orange-600':'text-slate-400'}`}>交易所</button></div>
             <div><label className={styles.label}>平台名稱</label><input placeholder="例如: Firstrade, 幣安" className={styles.input} value={name} onChange={e=>setName(e.target.value)} /></div>
             <div className="grid grid-cols-2 gap-3">
                 <div><label className={styles.label}>{editData ? '目前現金餘額' : '初始現金餘額'}</label><input type="number" className={styles.input} value={initialCash} onChange={e=>setInitialCash(e.target.value)} /></div>
                 <div><label className={styles.label}>幣別</label><select className={styles.input} value={currency} onChange={e=>setCurrency(e.target.value)}><option>USD</option><option>TWD</option><option>USDT</option></select></div>
             </div>
             <div className="flex gap-3 pt-2"><button onClick={onClose} className={styles.btnSecondary}>取消</button><button onClick={handleSave} className={`${styles.btnPrimary} flex-1`}>儲存</button></div>
         </div>
      </div></div>
...
export const ManagePlatformCashModal = ({ platform, userId, onClose }: any) => {
   ...
   return (
      <div className={styles.overlay}><div className={styles.content}>
         <h3 className="font-bold text-xl mb-4">管理 {platform.name} 現金</h3>
         <div className="bg-slate-50 p-3 rounded-xl mb-4 text-center">
             <div className="text-xs text-slate-400">目前餘額</div>
             <div className="text-2xl font-bold text-slate-800">{platform.balance.toLocaleString()} {platform.currency}</div>
         </div>
         <div className="space-y-4">
             <div className="flex bg-slate-100 p-1 rounded-xl"><button onClick={()=>setType('deposit')} className={`flex-1 py-2 rounded-lg text-sm font-bold ${type==='deposit'?'bg-white shadow text-emerald-600':'text-slate-400'}`}>入金 (Deposit)</button><button onClick={()=>setType('withdraw')} className={`flex-1 py-2 rounded-lg text-sm font-bold ${type==='withdraw'?'bg-white shadow text-red-600':'text-slate-400'}`}>出金 (Withdraw)</button></div>
             <div><label className={styles.label}>金額</label><input type="number" className={styles.input} value={amount} onChange={e=>setAmount(e.target.value)} /></div>
             <div className="flex gap-3 pt-2"><button onClick={onClose} className={styles.btnSecondary}>取消</button><button onClick={handleSave} className={`${styles.btnPrimary} flex-1`}>確認</button></div>
         </div>
      </div></div>
   )
}

// --- Assets (Updated with Platform Select & Cash Deduct) ---
export const AddAssetModal = ({ userId, platforms, onClose }: any) => {
   ...
   const handleSave = async () => {
      ...
      await addDoc(collection(db, getCollectionPath(userId, null, 'holdings')), {
         symbol: symbol.toUpperCase(), quantity: parseFloat(qty), avgCost: parseFloat(cost), currentPrice: parseFloat(cost),
         currency: platform?.currency || 'USD', type, platformId
      });

      // Deduct Cash from Platform if checked
      if(deductCash && platform) {
          await updateDoc(doc(db, getCollectionPath(userId, null, 'platforms'), platformId), {
              balance: platform.balance - totalCost
          });
      }
      onClose();
   };
   return (
      <div className={styles.overlay}><div className={styles.content}>
         ...
         <div className="space-y-4">
            ...
            {platformId && <div className="flex items-center gap-2 px-2">
                <input type="checkbox" checked={deductCash} onChange={e=>setDeductCash(e.target.checked)} id="dc" className="w-4 h-4 text-indigo-600 rounded"/>
                <label htmlFor="dc" className="text-xs text-slate-600 font-bold">從平台現金扣款 (總額: {(parseFloat(cost||'0')*parseFloat(qty||'0')).toFixed(2)})</label>
            </div>}
            <div className="grid grid-cols-2 gap-3">
               <div><label className={styles.label}>代號</label><input placeholder="AAPL" className={styles.input} value={symbol} onChange={e=>setSymbol(e.target.value)} /></div>
            ...
            <div><label className={styles.label}>平均單價 (Cost)</label><input type="number" className={styles.input} value={cost} onChange={e=>setCost(e.target.value)} /></div>
            <div className="flex gap-3 pt-2"><button onClick={onClose} className={styles.btnSecondary}>取消</button><button onClick={handleSave} className={`${styles.btnPrimary} flex-1`}>儲存</button></div>
         </div>
      </div></div>
   )
...
export const EditAssetModal = ({ holding, userId, onClose, onDelete }: any) => {
   ...
   const [cost, setCost] = useState(holding.avgCost.toString());

   const handleSave = async () => {
      await updateDoc(doc(db, getCollectionPath(userId, null, 'holdings'), holding.id), {
         quantity: parseFloat(qty), currentPrice: parseFloat(price), avgCost: parseFloat(cost)
      });
      onClose();
   };
   ...
   return (
      <div className={styles.overlay}><div className={styles.content}>
         <div className="flex justify-between mb-4"><h3 className="font-bold text-xl">編輯資產: {holding.symbol}</h3><button onClick={()=>onDelete(holding)} className="text-red-500 p-1 bg-red-50 rounded"><Trash2 size={18}/></button></div>
         <div className="space-y-4">
            <div><label className={styles.label}>目前市價 (手動更新)</label><input type="number" className={styles.input} value={price} onChange={e=>setPrice(e.target.value)} /></div>
            <div className="grid grid-cols-2 gap-3">
               ...
               <div><label className={styles.label}>平均成本</label><input type="number" className={styles.input} value={cost} onChange={e=>setCost(e.target.value)} /></div>
            </div>
            <div className="flex gap-3 pt-2"><button onClick={onClose} className={styles.btnSecondary}>取消</button><button onClick={handleSave} className={`${styles.btnPrimary} flex-1`}>儲存變更</button></div>
         </div>
      </div></div>
   )
}
...
export const TransferModal = ({ userId, accounts, onClose }: any) => {
  ...
  const handleTransfer = async () => {
    ...
    // Create two logs
    await addDoc(col, { accountId: fromId, type: 'out', amount: val, description: `轉帳至 ${accounts.find((a:any)=>a.id===toId)?.name}`, date: timestamp });
    await addDoc(col, { accountId: toId, type: 'in', amount: val, description: `來自 ${accounts.find((a:any)=>a.id===fromId)?.name} 轉入`, date: timestamp });
    onClose();
  }

  return (
     <div className={styles.overlay}><div className={styles.content}>
        <h3 className="font-bold text-xl mb-4">銀行轉帳</h3>
        <div className="space-y-4">
           <div className="flex items-center gap-2">
              <select className={styles.input} value={fromId} onChange={e=>setFromId(e.target.value)}><option value="">選擇轉出帳戶</option>{accounts.map((a:any)=><option key={a.id} value={a.id}>{a.name}</option>)}</select>
              <ArrowRightLeft className="text-slate-400" />
              <select className={styles.input} value={toId} onChange={e=>setToId(e.target.value)}><option value="">選擇轉入帳戶</option>{accounts.map((a:any)=><option key={a.id} value={a.id}>{a.name}</option>)}</select>
           </div>
           <div><label className={styles.label}>金額</label><input type="number" className={styles.input} value={amount} onChange={e=>setAmount(e.target.value)} /></div>
           <div><label className={styles.label}>日期</label><input type="date" className={styles.input} value={date} onChange={e=>setDate(e.target.value)} /></div>
           <div className="flex gap-3 pt-2"><button onClick={onClose} className={styles.btnSecondary}>取消</button><button onClick={handleTransfer} className={`${styles.btnPrimary} flex-1`}>確認轉帳</button></div>
        </div>
     </div></div>
  )
}

// --- Bank Detail (Editable & Grouped) ---
export const BankDetailModal = ({ userId, account, onClose, logs }: any) => {
   ...
   const addLog = async () => { 
     ...
     await addDoc(collection(db, getCollectionPath(userId, null, 'bankLogs')), { 
       accountId: account.id, type, amount: parseFloat(amt), description: desc || (type==='in'?'存款':'提款'), date: Timestamp.fromDate(new Date(date)) 
     }); 
     setAmt(''); setDesc(''); 
   }
   ...
   const deleteLog = async (e: React.MouseEvent, id: string) => {
      e.stopPropagation(); 
      if(window.confirm('確定要刪除此筆銀行交易紀錄嗎？(需二次確認)')) {
         await deleteDoc(doc(db, getCollectionPath(userId, null, 'bankLogs'), id));
      }
   }

   // Group logs by YYYY-MM
   const groupedLogs: Record<string, any[]> = {};
   logs.sort((a:any,b:any)=>(b.date?.seconds||0)-(a.date?.seconds||0)).forEach((l:any) => {
   ...
   return (
      <div className={styles.overlay}>
         <div className={`${styles.content} h-[80vh]`}>
            <div className="flex justify-between items-center mb-6">
               <div><h3 className="font-bold text-xl text-slate-800">{account.name}</h3><div className="text-sm text-slate-500">餘額: {account.currency} {account.currentBalance?.toLocaleString()}</div></div>
               <button onClick={onClose}><X size={20}/></button>
            </div>
            <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 mb-6">
               <div className="flex gap-2 mb-3"><button onClick={()=>setType('out')} className={`flex-1 py-2 rounded-md text-sm font-bold ${type==='out'?'bg-white shadow text-red-600':'text-slate-400'}`}>支出</button><button onClick={()=>setType('in')} className={`flex-1 py-2 rounded-md text-sm font-bold ${type==='in'?'bg-white shadow text-emerald-600':'text-slate-400'}`}>存入</button></div>
               <div className="flex gap-2 mb-2"><div className="w-32"><label className={styles.label}>日期</label><input type="date" className="w-full p-2 rounded border text-sm" value={date} onChange={e=>setDate(e.target.value)}/></div><div className="flex-1"><label className={styles.label}>金額</label><input type="number" className="w-full p-2 rounded border text-sm" value={amt} onChange={e=>setAmt(e.target.value)}/></div></div>
               <div className="flex gap-2 items-end"><div className="flex-1"><label className={styles.label}>說明</label><input className="w-full p-2 rounded border text-sm" value={desc} onChange={e=>setDesc(e.target.value)}/></div><button onClick={addLog} className="bg-indigo-600 text-white px-4 py-2 rounded font-bold text-xs h-[38px]">新增</button></div>
            </div>
            <div className="space-y-4 max-h-[50vh] overflow-y-auto">
               {Object.entries(groupedLogs).map(([month, monthLogs]) => (
                   <div key={month}>
                       <div className="text-xs font-bold text-slate-400 mb-2 bg-slate-50 p-1 rounded w-fit">{month}</div>
                       <div className="space-y-2">
                           {monthLogs.map((l:any) => (
                              <div key={l.id} className="flex justify-between items-center p-3 bg-white rounded-xl border border-slate-100">
                                 <div className="flex items-center gap-3">
                                    <div className={`p-2 rounded-full ${l.type==='in'?'bg-emerald-100 text-emerald-600':'bg-red-100 text-red-600'}`}>{l.type==='in'?<ArrowDownRight size={14}/>:<ArrowUpRight size={14}/>}</div>
                                    <div><div className="font-bold text-slate-700 text-sm">{l.description}</div><div className="text-[10px] text-slate-400">{l.date?.seconds ? new Date(l.date.seconds*1000).toLocaleDateString() : ''}</div></div>
                                 </div>
                                 <div className="flex items-center gap-3"><div className={`font-bold font-mono ${l.type==='in'?'text-emerald-600':'text-red-600'}`}>{l.type==='in'?'+':''}{l.amount}</div><button onClick={(e)=>deleteLog(e, l.id)} className="text-slate-300 hover:text-red-500"><Trash2 size={14}/></button></div>
                              </div>
                           ))}
                       </div>
                   </div>
...
export const AddAccountModal = ({userId, onClose, editData}:any) => {
   const [name,setName]=useState(editData?.name || ''); 
   const [bal,setBal]=useState(editData?.initialBalance?.toString() || ''); 
   const [cur,setCur]=useState(editData?.currency || 'TWD');

   const handleSave = async () => {
      const col = collection(db,getCollectionPath(userId,null,'accounts'));
      if (editData) {
         await updateDoc(doc(col, editData.id), { name, currency: cur, initialBalance: parseFloat(bal)||0 });
      } else {
         await addDoc(col,{name,initialBalance:parseFloat(bal)||0,currency:cur,currentBalance:parseFloat(bal)||0});
      }
      onClose();
   }
...
export const AddCardModal = ({userId, onClose, editData}:any) => {
   const [name,setName]=useState(editData?.name || ''); 
   const [day,setDay]=useState(editData?.billingDay?.toString() || '1');

   const handleSave = async () => {
      const col = collection(db,getCollectionPath(userId,null,'creditCards'));
      if (editData) {
         await updateDoc(doc(col, editData.id), { name, billingDay: parseInt(day) });
      } else {
         await addDoc(col,{name,billingDay:parseInt(day)});
      }
      onClose();
   }
